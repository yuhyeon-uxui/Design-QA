...
class OptionsUtil {
    ...
    _getKnownOptions(options, context) {
        const knownOptions = commonHgZlEz_K_node.ObjectValue.empty();
        ...
        for (const knownOptionKey in this.optionDefinitions) {
            if (this.optionDefinitions.hasOwnProperty(knownOptionKey)) {
                ...
                if (knownOptionKey in options) {
                    ...
                    if (optionDefinition.nestedOptions && commonHgZlEz_K_node.isPlainObject(optionValue)) {
                        ...
                        protoValue = commonHgZlEz_K_node.parseData(optionValue, context) ?? undefined;
                    }
                    if (protoValue) {
                        knownOptions.set(commonHgZlEz_K_node.FieldPath$1.fromServerFormat(optionDefinition.serverName), protoValue);
                    }
                }
            }
        }
    ...
    getOptionsProto(context, knownOptions, optionsOverride) {
        ...
        if (optionsOverride) {
            const optionsMap = new Map(commonHgZlEz_K_node.mapToArray(optionsOverride, (value, key) => [
                commonHgZlEz_K_node.FieldPath$1.fromServerFormat(key),
                value !== undefined ? commonHgZlEz_K_node.parseData(value, context) : null
            ]));
...
function isIMapValue(obj) {
    ...
    if ('fields' in obj && (obj.fields === null || commonHgZlEz_K_node.isPlainObject(obj.fields))) {
        return true;
    }
    return false;
}
...
function valueToDefaultExpr$1(value) {
    ...
    else if (commonHgZlEz_K_node.isPlainObject(value)) {
        result = _map(value);
    }
...
function vectorToExpr$1(value) {
    ...
    else if (value instanceof commonHgZlEz_K_node.VectorValue) {
        return constant(value);
    }
    else if (Array.isArray(value)) {
        return constant(commonHgZlEz_K_node.vector(value));
    }
    else {
...
function fieldOrExpression$1(value) {
    if (commonHgZlEz_K_node.isString(value)) {
...
class Expression {
    ...
    asBoolean() {
        if (this instanceof BooleanExpression) {
            ...
            throw new commonHgZlEz_K_node.FirestoreError('invalid-argument', `Conversion of type ${typeof this} to BooleanExpression not supported.`);
        }
    }
    subtract(subtrahend) {
...
function _field(nameOrPath, methodName) {
    if (typeof nameOrPath === 'string') {
        if (commonHgZlEz_K_node.DOCUMENT_KEY_NAME === nameOrPath) {
            return new Field(commonHgZlEz_K_node.documentId()._internalPath, methodName);
        }
        return new Field(commonHgZlEz_K_node.fieldPathFromArgument('field', nameOrPath), methodName);
    }
    else {
...
class Constant extends Expression {
     ...
     */
    _toProto(_) {
        commonHgZlEz_K_node.hardAssert(this._protoValue !== undefined, 0x00ed);
    ...
    _readUserData(context) {
        ...
        if (isFirestoreValue(this._protoValue)) {
            ...
            this._protoValue = commonHgZlEz_K_node.parseData(this.value, context);
        }
    }
}
...
class MapValue extends Expression {
    ...
    _toProto(serializer) {
        return commonHgZlEz_K_node.toMapValue(serializer, this.plainObject);
    }
}
/**
 *
...
function timestampTruncate(fieldNameOrExpression, granularity, timezone) {
    const internalGranularity = commonHgZlEz_K_node.isString(granularity)
...
class PipelineValueExpression extends Expression {
    ...
    _toProto(jsonProtoSerializer) {
        return commonHgZlEz_K_node.toPipelineValue(this.pipeline._toProto(jsonProtoSerializer));
    }
    /**
     * @internal
     */
...
class Ordering {
    ...
    _toProto(serializer) {
        return {
            mapValue: {
                fields: {
                    direction: commonHgZlEz_K_node.toStringValue(this.direction),
...
function isSelectable(val) {
    const candidate = val;
    return (candidate.selectable && commonHgZlEz_K_node.isString(candidate.alias) && isExpr(candidate.expr));
}
...
function isAliasedAggregate(val) {
    const candidate = val;
    return (commonHgZlEz_K_node.isString(candidate.alias) &&
...
class AddFields extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...
            args: [commonHgZlEz_K_node.toMapValue(serializer, this.fields)]
        };
    }
    _readUserData(context) {
...
class Define extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonHgZlEz_K_node.toMapValue(serializer, this.aliasedExpressions)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Aggregate extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [
                commonHgZlEz_K_node.toMapValue(serializer, this.accumulators),
                commonHgZlEz_K_node.toMapValue(serializer, this.groups)
            ]
        };
    }
    _readUserData(context) {
...
class Distinct extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonHgZlEz_K_node.toMapValue(serializer, this.groups)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class DocumentsSource extends Stage {
    ...
    constructor(docPaths, options) {
        ...
        if (!docPaths || docPaths.length === 0) {
            throw new commonHgZlEz_K_node.FirestoreError(commonHgZlEz_K_node.Code.INVALID_ARGUMENT, 'Empty document paths are not allowed in DocumentsSource');
        }
        ...
        if (uniqueDocPaths.size !== paths.length) {
            throw new commonHgZlEz_K_node.FirestoreError(commonHgZlEz_K_node.Code.INVALID_ARGUMENT, 'Duplicate document paths are not allowed in DocumentsSource');
        }
    ...
    _toProto(serializer) {
        return {
            ...
            args: [
                ...
                commonHgZlEz_K_node.toStringValue(this.distanceMeasure)
            ]
        };
    }
    _readUserData(context) {
...
class Limit extends Stage {
    ...
    constructor(limit, options) {
        commonHgZlEz_K_node.hardAssert(!isNaN(limit) && limit !== Infinity && limit !== -Infinity, 0x882c);
        super(options);
        this.limit = limit;
    }
    /**
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonHgZlEz_K_node.toNumber(serializer, this.limit)]
        };
    }
}
class Offset extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonHgZlEz_K_node.toNumber(serializer, this.offset)]
        };
    }
}
class Select extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonHgZlEz_K_node.toMapValue(serializer, this.selections)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Sample extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonHgZlEz_K_node.toNumber(serializer, this.rate), commonHgZlEz_K_node.toStringValue(this.mode)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Union extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonHgZlEz_K_node.toPipelineValue(this.other._toProto(serializer))]
        };
    }
    _readUserData(context) {
...
class Replace extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [this.map._toProto(serializer), commonHgZlEz_K_node.toStringValue(Replace.MODE)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
function readUserDataHelper(expressionMap, context) {
    if (commonHgZlEz_K_node.isUserData(expressionMap)) {
...
function toPipelineBooleanExpr(f) {
    if (f instanceof commonHgZlEz_K_node.FieldFilter) {
        ...
        switch (f.op) {
            ...
            default:
                commonHgZlEz_K_node.fail(0x9047);
        }
    }
    else if (f instanceof commonHgZlEz_K_node.CompositeFilter) {
        switch (f.op) {
            ...
            default:
                commonHgZlEz_K_node.fail(0x89ea);
        }
    }
...
function toPipelineStages(query, db) {
    let pipeline;
    if (commonHgZlEz_K_node.isCollectionGroupQuery(query)) {
        pipeline = db.pipeline().collectionGroup(query.collectionGroup);
    }
    else if (commonHgZlEz_K_node.isDocumentQuery(query)) {
        pipeline = db.pipeline().documents([commonHgZlEz_K_node.doc(db, query.path.canonicalString())]);
    }
    else {
    ...
    // orders
    const orders = commonHgZlEz_K_node.queryNormalizedOrderBy(query);
...
function selectablesToObject(selectables) {
    ...
    for (const selectable of selectables) {
        ...
        if (typeof selectable === 'string') {
            ...
            commonHgZlEz_K_node.fail(0x5319, { selectable });
        }
        if (result[alias] !== undefined) {
            throw new commonHgZlEz_K_node.FirestoreError('invalid-argument', `Duplicate alias or field '${alias}'`);
        }
...
function aliasedAggregateToMap(aliasedAggregatees) {
    return aliasedAggregatees.reduce((map, selectable) => {
        if (map.get(selectable.alias) !== undefined) {
            throw new commonHgZlEz_K_node.FirestoreError('invalid-argument', `Duplicate alias or field '${selectable.alias}'`);
        }
...
function vectorToExpr(value) {
    if (value instanceof Expression) {
        ...
        const result = constant(commonHgZlEz_K_node.vector(value));
        return result;
    }
    else {
        throw new Error('Unsupported value: ' + typeof value);
...
class Pipeline {
    ...
    removeFields(fieldValueOrOptions, ...additionalFields) {
        // Process argument union(s) from method overloads
        const options = isField(fieldValueOrOptions) || commonHgZlEz_K_node.isString(fieldValueOrOptions)
            ? {}
            : fieldValueOrOptions;
        const fields = isField(fieldValueOrOptions) || commonHgZlEz_K_node.isString(fieldValueOrOptions)
            ? [fieldValueOrOptions, ...additionalFields]
            : fieldValueOrOptions.fields;
        // Convert user land convenience types to internal types
        const convertedFields = fields.map(f => commonHgZlEz_K_node.isString(f) ? field(f) : f);
        // Create stage object
    ...
    select(selectionOrOptions, ...additionalSelections) {
        // Process argument union(s) from method overloads
        const options = isSelectable(selectionOrOptions) || commonHgZlEz_K_node.isString(selectionOrOptions)
            ? {}
            : selectionOrOptions;
        const selections = isSelectable(selectionOrOptions) || commonHgZlEz_K_node.isString(selectionOrOptions)
    ...
    offset(offsetOrOptions) {
        ...
        let options;
        let offset;
        if (commonHgZlEz_K_node.isNumber(offsetOrOptions)) {
            options = {};
            offset = offsetOrOptions;
        }
        else {
    ...
    limit(limitOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonHgZlEz_K_node.isNumber(limitOrOptions) ? {} : limitOrOptions;
        const limit = commonHgZlEz_K_node.isNumber(limitOrOptions)
            ? limitOrOptions
    ...
    distinct(groupOrOptions, ...additionalGroups) {
        // Process argument union(s) from method overloads
        const options = commonHgZlEz_K_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
            ? {}
            : groupOrOptions;
        const groups = commonHgZlEz_K_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
    ...
    replaceWith(valueOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonHgZlEz_K_node.isString(valueOrOptions) || isExpr(valueOrOptions) ? {} : valueOrOptions;
        const fieldNameOrExpr = commonHgZlEz_K_node.isString(valueOrOptions) || isExpr(valueOrOptions)
            ? valueOrOptions
            : valueOrOptions.map;
        // Convert user land convenience types to internal types
    ...
    sample(documentsOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonHgZlEz_K_node.isNumber(documentsOrOptions) ? {} : documentsOrOptions;
        let rate;
        let mode;
        if (commonHgZlEz_K_node.isNumber(documentsOrOptions)) {
            rate = documentsOrOptions;
            mode = 'documents';
        }
        else if (commonHgZlEz_K_node.isNumber(documentsOrOptions.documents)) {
    ...
    unnest(selectableOrOptions, indexField) {
        ...
        if (commonHgZlEz_K_node.isString(indexFieldName)) {
...
class PipelineSource {
    ...
    collection(collectionOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonHgZlEz_K_node.isString(collectionOrOptions) ||
            commonHgZlEz_K_node.isCollectionReference(collectionOrOptions)
            ? {}
            : collectionOrOptions;
        const collectionRefOrString = commonHgZlEz_K_node.isString(collectionOrOptions) ||
            commonHgZlEz_K_node.isCollectionReference(collectionOrOptions)
            ? collectionOrOptions
            : collectionOrOptions.collection;
        // Validate that a user provided reference is for the same Firestore DB
        if (commonHgZlEz_K_node.isCollectionReference(collectionRefOrString)) {
            this._validateReference(collectionRefOrString);
        }
        // Convert user land convenience types to internal types
        const normalizedCollection = commonHgZlEz_K_node.isString(collectionRefOrString)
    ...
    collectionGroup(collectionIdOrOptions) {
        ...
        let collectionId;
        let options;
        if (commonHgZlEz_K_node.isString(collectionIdOrOptions)) {
    ...
    documents(docsOrOptions) {
        ...
        docs
            .filter(v => v instanceof commonHgZlEz_K_node.DocumentReference)
            .forEach(dr => this._validateReference(dr));
        // Convert user land convenience types to internal types
        const normalizedDocs = docs.map(doc => commonHgZlEz_K_node.isString(doc) ? doc : doc.path);
        // Create stage object
    ...
    _validateReference(reference) {
        ...
        if (!refDbId.isEqual(this.databaseId)) {
            throw new commonHgZlEz_K_node.FirestoreError(commonHgZlEz_K_node.Code.INVALID_ARGUMENT, `Invalid ${reference instanceof commonHgZlEz_K_node.CollectionReference
                ? 'CollectionReference'
                : 'DocumentReference'}. ` +
...
function subcollection(pathOrOptions) {
    ...
    let path;
    let options;
    if (commonHgZlEz_K_node.isString(pathOrOptions)) {
        path = pathOrOptions;
        options = {};
    }
    else {
...
class PipelineResult {
    ...
    get(fieldPath) {
        ...
        const value = this._fields.field(commonHgZlEz_K_node.fieldPathFromArgument('DocumentSnapshot.get', fieldPath));
        if (value !== null) {
...
function execute(pipeline) {
    if (!pipeline._db) {
        return Promise.reject(new commonHgZlEz_K_node.FirestoreError(commonHgZlEz_K_node.Code.FAILED_PRECONDITION, 'This pipeline was created without a database (e.g., as a subcollection pipeline) and cannot be executed directly. It can only be used as part of another pipeline.'));
    }
    const datastore = commonHgZlEz_K_node.getDatastore(pipeline._db);
    const firestore = commonHgZlEz_K_node.cast(pipeline._db, commonHgZlEz_K_node.Firestore);
    const userDataReader = commonHgZlEz_K_node.newUserDataReader(firestore);
    ...
    return commonHgZlEz_K_node.invokeExecutePipeline(datastore, structuredPipeline).then(result => {
        ...
        const docs = result
            ...
            ? new commonHgZlEz_K_node.DocumentReference(firestore, null, element.key)
 ...
 * ```
 */
commonHgZlEz_K_node.Firestore.prototype.pipeline = function () {
    const userDataWriter = new commonHgZlEz_K_node.LiteUserDataWriter(this);
    const userDataReader = commonHgZlEz_K_node.newUserDataReader(this);
